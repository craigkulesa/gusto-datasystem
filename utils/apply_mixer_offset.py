...
def get_cal_mixer_offsets(gustodir):
    f = open(f'{gustodir}/calib/offsets.txt','r')
    offsets=[]
    azoffs=[]
    eloffs=[]
    bm = []
...
def updatePosBandMixer(hdr0, ra, dec,otime, band, mix, verbose=False):
    """
    Function calculating the mixer pointings using offsets from the fiducial pixel.  
    ...
    azoffset, aloffset, bm = get_cal_mixer_offsets('/data/scratch/GUSTO/gusto-datasystem/')
    # get beam offsets:
    ...
    # apply beam offsets
    naz = altaz.az.deg*u.deg + azoffm*u.deg
    nalt = altaz.alt.deg*u.deg + aloffm*u.deg
...
def main(args=None,verbose=True):
    if args==None:
        ...
        my_parser = argparse.ArgumentParser(prog='apply_mixer_offset',
                                            usage='%(prog)s filetype directory ',
                                            description='Update RA/DEC positions in GUTSO data file')
        my_parser.version = "Version 0.0.1 (22 Oct 2024) "
        my_parser.add_argument('-v', action='version')
        my_parser.add_argument('fileroot',
                               metavar='fileroot',
                               type=str,
    ...
    files = sorted(glob.glob(f'{directory}/{fileroot}*.fits'))
    ...
    for file in files:
        ...
        history_phrase = f'Mixer Offsets Applied'
        ...
        if not in_history:
            ...
            for mx in mixer:
                ...
                nradec = updatePosBandMixer(hdr0, mra, mdec, obstime, band, mx, verbose=False)
                newra  = nradec.ra.deg
            ...
            print(f'Offsets allready applied: {file} not updated)')
